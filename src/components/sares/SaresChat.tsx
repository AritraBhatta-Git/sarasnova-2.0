...
export function SaresChat() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 shadow-xl backdrop-blur-2xl overflow-hidden font-sans">
      {/* Header Bar */}
      <div className="px-5 py-3.5 bg-slate-100 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative w-9 h-9 rounded-xl bg-indigo-600 flex items-center justify-center font-bold text-white">
            ...
            <div
              className={`max-w-[85%] p-3.5 rounded-2xl ${
                msg.sender === "user"
                  ...
                  : "bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 rounded-tl-none space-y-3"
              }`}
            >
              ...
              {/* Action Button */}
              {msg.actionBtn && (
                <button className="w-full py-2 px-3 rounded-xl bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 text-white font-semibold text-[11px] flex items-center justify-center gap-1.5 transition-colors">
      ...
      {/* Quick Prompt Chips */}
      <div className="px-5 py-2 bg-slate-100 dark:bg-slate-900/60 border-t border-slate-200 dark:border-slate-800 flex items-center gap-2 overflow-x-auto no-scrollbar text-[10px]">
        ...
        {samplePrompts.map((prompt, pIdx) => (
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/20 shrink-0 transition-colors"
          >
            {prompt}
          </button>
        ))}
      ...
      <form onSubmit={handleSend} className="p-3 bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center gap-2">
        <input
          ...
          className="flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500"
        />
        <button
          type="submit"
          className="p-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-colors shrink-0"
